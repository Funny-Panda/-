...
public class SyntaxAnanlyzer {
	...
	public void getText() {
		...
		try {
			...
			while ((lString = bufferedReader.readLine()) != null) {
				...
				stack.push('$');
				stack2.push(0);
				output.write("栈\t\t\t符号\t\t\t输入\t\t\t动作");
				output.newLine();
				boolean b = analysis();
				if (b) {
					output.write("符合设定的LR(1)文法");
					output.newLine();
					output.write("自下而上产生式顺序如下：");
					output.newLine();
	...
	public void display() {// 读SymbolStack、StateStack和input里的所有字符，显示到输出文件
		...
		try {
			output.write(states + "\t\t\t" + symbols + "\t\t\t" + input + "\t\t\t" + actions);
			output.newLine();
		} catch (IOException e) {
			e.printStackTrace();
		}
...
//				actions += "Reduce by production " + s;
				actions += "根据" + production.getProduction(n) + "规约";
				display();
